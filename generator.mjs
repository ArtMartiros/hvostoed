...
/* Поворот: плитка пола, соединяющая две стороны клетки из четырёх. Луч, вошедший
   ...
   Стенки плитки не зависят от того, кто на ней лежит: закрытая сторона читается
   РАНЬШЕ занятости, а у головы, лежащей на плитке, — до первого же шага луча.
   Перекрывается занятостью только ИЗГИБ: он выбирает выход по стороне входа,
   а её нет ни у луча, остановленного чужим телом, ни у луча из своей же клетки. */
/* Звёзды — это отметки ДЛИНЫ, и причина измеренная. Ход в «Хвостоеде» всегда
   убирает с доски ровно одну змею (обед — жертву, вылет — себя; проверено полным
   перебором на всех достижимых состояниях 44 уровней, нарушений 0). Значит «за
   минимум ходов» — это «оставь на доске побольше змей», то есть ровно наоборот к
   «съедено всё»; отдельной оси «эффективность» тут нет. Хуже того: на 35 уровнях
   из 44 ВСЕ победные концовки одинаковы по числу змей, длине и потерям, поэтому
   любая звезда за состояние доски меряет уровень, а не игрока. Прежняя схема этим
   и болела: «никто не улетел» недостижима на 11 уровнях (вылет там обязателен по
   построению), «осталась одна змея» — на 12 (колючую не съесть никогда), а три
   звезды сразу были возможны лишь на 18 из 44.
   Длина же — шкала, живая на всех уровнях: верхняя отметка равна измеренному
   потолку поля, значит достижима по построению, а не предположительно. Нижняя
   обязана быть выше стартовой длины — иначе звезда горит до первого тапа.

   Половина и три четверти — не круглые числа с потолка, а замер. Обед глотает змею
   ЦЕЛИКОМ, поэтому длина растёт большими скачками, и отметка, поставленная близко к
   потолку, почти никогда не оказывается ИСХОДОМ: при 0.65/0.85 случайная партия
   заканчивалась на двух звёздах в 5% случаев (и хотя бы в 10% — лишь на 7 уровнях
   из 44), при 0.5/0.75 — в 17% (на 24 уровнях). Те же доли уже стояли на полях
   рекорда, так что теперь вся игра меряется одной линейкой. */
...
export function raycast(state, i, w, h, board) {
  ...
  for (let step = 0; ; step++) {
    ...
    // значит лёгшая на вход змея его перекрывает — то же правило, что у поворота.
...
/* ---------- перебор способов отменить обед ----------
   ...
   С поворотами прямизна нужна только НА ВХОДЕ — прицел едока смотрит в первую
...
/* Совместим ли вариант с полом, который уже сложился: на клетке либо плитка, либо
   ...
   Спина поворота стена, и лежащая на плитке змея её не отменяет, — значит плитка
   ...
   выбор это почти не сужает (замер: повороты доезжают в 98–100% сборок). */
...
export function generate(cfg) {
  ...
  for (let step = 0; step < M; step++) {
    ...
    for (const si of order)
      for (const opt of shuffled(rnd, splitOptions(state[si], cfg.maxGap, wantApple ? 1 : 2, gates)))
        cand.push({ si, opt });
    // при равном зазоре предпочитаем вариант, требующий меньше новых поворотов:
    // прямой луч читается быстрее, повороты нужны там, где без них не выйдет.
    // Но пока заказанные повороты НЕ НАБРАНЫ, предпочтение обратное: cfg.turns
    // раньше работал одним лишь потолком, никто к нему не стремился, и ручка
    // «Поворотов 3» отдавала в среднем 1.9. Знак сравнения — вся разница.
  ...
  /* Мосты. В обратном построении клетки зазоров обязаны оставаться пустыми — их и
     ...
     ПЕРВЫМ: раньше занятости, раньше портала, раньше поворота. Значит мост — не
     «клетка, где не читается занятость», а «клетка, где не читается ничего», и
     запретов ему нужно четыре, а не два:
       · занято в начальном состоянии, и плитка поворота — были и раньше;
