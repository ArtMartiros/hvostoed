/* Полоса G «Комнат»: плотные уровни обратным построением (прямой строитель в
   тесноте захлёбывается). Финал комнаты = новая масса ++ чемпион, режется
   обратными ходами; разрезы впритык коммутируют и проходятся жадным тапом,
   поэтому каждая комната со второй несёт ловушку planTrap (замурованная линия),
   и приёмка требует провала жадной пробы. Замеры — бриф §6ab. */
import fs from 'fs';
...
/* ---------- путь новой массы ----------
   От хвоста (куда смотрит чемпион) серпантином по комнате; голова пути — голова
   чемпиона следующей стадии, тянется к её двери и не смотрит в тело/крышку. */
  ...
  for (let attempt = 0; attempt < 40; attempt++) {
    ...
    while (cells.length < len) {
      ...
      if (!inRect(rect, cur)) {
        ...
        // прямой шаг в приоритете: серпантин даёт складки (близко на доске,
        // далеко по пути) для ловушек; кольцо по периметру складок не даёт
...
/* Прямой ли разрез пути cells на позиции b с зазором k: едок cells[b+k] обязан
   ...
   прямой (поворотов в комнатах нет, гнуть путь нечем). */
export function straightCut(cells, b, k) {
  ...
  /* t доходит до b ВКЛЮЧИТЕЛЬНО: последний шаг — от зазора к хвосту жертвы —
     ...
     С «t > b» зазоры садились на изгибы пути, жертва выпадала из линии хода,
...
/* Раскрой массы на куски 2..5 с зазорами из бюджета: зазор оставляет пустые
   линии обстрела, в которые свешиваются хвосты подразрезов, — так рождаются
   запирающие порядки. Возвращает [{start, size, gap}] от головы пути. */
...
/* Целевая ловушка планируется ДО раскроя (случайно складывалась ни разу):
   ищется складка серпантина, хвост висящего куска E доращивается в линию,
   которую чемпион стреляет позже, — съешь E раньше времени, и хвост замурует
   линию навсегда; правильный ход — дать E съесть свою жертву и уползти. */
...
/* Обратный ход на куске: жертва — голова куска, едок — остаток с дорощенным
   хвостом. Зазор — правило (80% разрезов), не приправа: разрезы впритык
   коммутируют, и жадный тап проходил комнату насквозь; хвосту нельзя в
   собственный зазор (вперёд-ход идёт по нему). */
function generalCut(rnd, cells, occNow, W, H, magnets) {
  ...
  let ext = [];
  if (k > 0) {
    // магниты — зазоры чужих цепочек: берём доращивание, чей кончик лёг туда
    // (случайный хвост попадает в линию слишком редко)
...
export function buildStageReverse(rnd, champion, rect, band, ctx, W, H, doorRect, wantSub) {
  ...
  // комната с чемпионом несёт целевую ловушку (иначе жадный тап проходит её
  // насквозь); сдвиг чемпиона при зазоре — по его же телу, в данные не попадает;
  // первая комната безопасна — простой раскрой без зазоров
