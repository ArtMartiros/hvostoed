...
/* У generator.mjs в boardOf НЕТ валунов — прототипный стенд молча гонял ходы
   ...
   Она не ходит (sleep), её не съесть (spiky), об её тело упираются — ровно
...
/* Направленное построение стадии: случайная посыпка не складывается в цепочку
   ...
   руками: каждая новая змея садится либо ХВОСТОМ на текущий путь большой змеи
...
function buildStage(rnd, champion, rect, n, slen, ctx, W, H) {
  ...
  const forbidden = new Set();       // клетки запланированных путей — не застраивать
  ...
  /* пришить к змее base новую: base ест новую (её хвост на пути base) или
  ...
  // сшить cur и local, если кто-то уже достаёт до чужого хвоста
  const join = (a, bSnake) => {
  ...
  /* Чемпион входит в комнату не всегда удачно: его путь и хвост могут не
     дотягиваться. Тогда в комнате растёт СВОЯ цепочка, и сшиваем её с большой,
     как только чьи-то путь и хвост совпадут. В первой комнате большой змеи нет —
  ...
  for (let guard = 0; guard < 60; guard++) {
    ...
    if (remaining === 0) return null;          // осталось только сшить, а пути не сходятся
...
const LESSONS = {
  A: ['Собери всех змей комнаты в одну — откроется следующая.',
      ...
      'Закрытая комната — стена: змея в неё упирается, как в валун.',
