...
function raycast(snakes, i, W, H, board) {
  const s = snakes[i];
  let [dx, dy] = facing(s.cells);          // поворот гнёт ход, поэтому не const
  const occ = occMap(snakes);
  let [x, y] = s.cells[0];
  const gap = [];
  /* Голова САМА может лежать на плитке поворота, и тогда змея трогается ИЗНУТРИ
     жёлоба. Клетку головы цикл не читает — он начинает с шага, — поэтому свою
     плитку проверяем отдельно, иначе стенка держала только снаружи, а
     изнутри он уходил сквозь неё и съедал хвост за стеной.
     Гнуть тут нечего: выход поворота выбирается по стороне ВХОДА, а у хода,
     начатого внутри плитки, её нет. Открытая сторона выпускает, закрытая
  ...
  for (let step = 0; ; step++) {
    ...
    if (board.bridges.has(ck(x, y))) { gap.push([x, y]); continue; }   // над мостом ход не останавливается
    ...
    // легшая на плитку, подставляла свой хвост под ход, который упирался повороту
