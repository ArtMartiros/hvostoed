...
function turnWallCase() {
  ...
  const cases = [
    ...
    ['голова лежит в жёлобе и смотрит в открытую сторону — змея выходит как обычно',
      [[[2, 1], [2, 2]], [[3, 0], [2, 0]]], 'tail'],
    ['змея снаружи бьёт плитке в спину, а на плитке лежит хвост',
...
/* Отдельным прогоном — доска, где включено ВСЁ сразу и погуще, чем в пресетах:
   тест обязан хоть раз провести ход через портал, мост и поворот на одной доске.
...
const CASES = Object.keys(PRESETS).filter((n) => !PRESETS[n].record).map((n) => [n, PRESETS[n]])
  ...
  /* И отдельно — доска, где колючая не приманка, а та, что ест ПОСЛЕДНЕЙ. Колючих
     ...
     чужие ходы, упирающиеся в её хвост, и разойдись игра с генератором хоть на одном,
  ...
  /* И отдельно — ЛОЖНАЯ ВЕТКА. Её обещание держится на одном свойстве обеда: голова
     ...
     путей работает как цепочка ходов. Свойство это живёт в applyEat, копий у applyEat
